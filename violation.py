...
def exists2(conn, table, field, value):
    if type(value) == int:
        value = str(value)
    elif type(value) == str:
        value = "'" + value + "'"
    ...
    curs = conn.cursor()
    curs.execute("select owner_id from " + table + 
        " where is_primary_owner = 'y' and " + field + " = " + value)
    i = curs.fetchone()[0]
    curs.close()
    
    return i
	
def primary_owner_ticket(conn):
        
    while True:
        print("Enter the Vehicle ID of the violator")
        vehicle_id = common.read_string('serial_no', 15)
        if vehicle_id == None:
            return None
        if common.exists(conn, 'vehicle', 'serial_no', vehicle_id):
            break
        print('An automobile with this vehicle ID does not exists!')
        
    while True:
        print("Checking if vehicle has any primary owner registered.")
        try:
            violator_no = exists2(conn, 'owner', 'vehicle_id', vehicle_id)
            print(violator_no)
            break
        except ValueError:
            print('A primary owner for this vehicle does not exists, exiting this option!')
            return None
